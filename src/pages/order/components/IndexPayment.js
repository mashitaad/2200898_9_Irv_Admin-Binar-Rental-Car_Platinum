  ...
  const handleDelete = (payload) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire('dikonfirmasi!', 'Pesanan Anda Berhasil dibatalkan.', 'success').then(() => {
          dispatch(adminGetAllOrder(DEFAULT_FILTER));
        });
      }
    });
  ...
  const handleConfirm = (payload) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire('dibatalkan!', 'Pesanan Anda Berhasil dikonfirmasi.', 'success').then(() => {
          dispatch(adminGetAllOrder(DEFAULT_FILTER));
        });
      }
    });
      ...
      <div>
        <Tabs defaultActiveKey="payment-all" id="justify-tab-example" className="mb-3" justify>
          ...
          <Tab eventKey="payemnt-onprocces" title="Perlu Diproses">
            <OnProcessPaymentStatus
              data={filteredOnProcessOrder}
              handleDelete={handleDelete}
              handleConfirm={handleConfirm}
            />
          </Tab>
